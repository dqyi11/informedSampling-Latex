...
\begin{prop}
	\label{prop:asym_opt}
	\dy{claim that once informed sampler provides uniform samplings in any given level set in a non-euclidean state space, the corresponding informed RRT* could asymptotically find an optimal solution.}
\end{prop}
...
\os{refer to Fig.~2 or the updated one.}
...
\dy{Do you think we should include some figures that has been commented out below to show how difficult sampling an informed set is?}
...
See Fig.~\ref{fig:cost_surface_1d} for a side view of the cost surface for one degree of freedom ($\left(x, \dot{x}\right)$). One can see a key property of the cost surface that cause serious concerns is the discontinuous nature of the surface. 
As mentioned before, finding the minimum time problem amounts to solving a quadratic equation. 
Because the nature of this equation changes with the start and end goal, there is a curve in the input space that amounts to a switching surface that represents the boundary for the decision of the sign of $a_1$. 
This surface causes a discontinuous cost surface as shown in Fig. ~\ref{fig:cost_surface_1d}.
}
...
\dy{Rohan, please finish this part}

Also we can take other approaches that generate samples from a Markov chain without requiring an approximation of a target distribution $ \pi $.
Hit-and-Run \cite{KSZ11} is one of them that efficiently generates uniform samples in convex regions.
In order to generalize our sampler to a wider range of problems~\cite{KSZ11} and better efficiency
...
\dy{say some Hit-and-Run's advantages over Metropolis-Hastings}
...
\dy{shall we use informed set or level set?}

\dy{going to change the tick labels so this below sentence will be replaced}
...
A small covariance tends to generate more samples near around previous samples, which a large covariance is more likely to drive a Markov chain outside of an informed set.
...
For example, a sampler that constantly only return the same sample in a level set might show the best sampling efficiency can be the worst sampler in a path planning problem.
...
Also, HNR also show fast convergence to the optimal, because it is better at adapting the informed set update in planning process. 
