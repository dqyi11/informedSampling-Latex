...
Moreover, this hyperspheroid is defined only be the cost $c_{\text{best}}$ of the current best solution and not by the location of the start~$x_s$ and goal~$x_g$.
...
To understand the differences recall that optimal paths for MTDI follow a ``bang-bang`` controller~\cite{HN10, KS14}.
...
We start  by randomly sampling a configuration~$x_\text{rand} \in \calX$.
This sample~$x_\text{rand}$ is used to initialize a gradient-descent algorithm which produces a sample~$x_\text{boundary} \in \partial\Cinf$ which lies (approximately) on the boundary of the informed set (Sec.~\ref{subsec:grad}).
...
In our setting, we chose to skip this 

Recall (Sec.~\ref{sec:mtdi}) that our cost function is nor necessarily continuous
...
$
penalizes states outside the informed set~$\Cinf$ using a tight sigmoid function~$\text{sigm}$ scaled by some (large\footnote{The constant~$\beta_{\text{informed}}$ gives points out of~\Cinf a large energy value which in turn implies a low probability of being sampled. In theory, we would like to have~$\beta_{\text{informed}} \approx \infty$. However, choosing~$\beta_{\text{informed}}$ to be to large may cause numerical instabilities.}) constant~$\beta_{\text{informed}}$.
