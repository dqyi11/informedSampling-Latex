...
Finally, reachability-guided sampling~\cite{SWT09, PLAEFRA17} focuses on sampling regions of the state space that are most likely to promote expansion under the differential constraints.
...
Given a cost function $C : \calX \times \calU \rightarrow \R$, the cost of a trajectory $ \gamma $ is defined as the accumulated cost along the path
...
Notice that~$ \gamma^*(x) $ is not required to be collision free.
...
To understand why we resort to optimization-based methods and do not attempt to provide a closed-form solution to sample $\Cinf$ we study the structure of the informed set for a simple yet important dynamical system---the double integrator. 
...
(i)~the structure of~\Cinf changes not only with~$\hat{c}$ but also according to the specific values of the start~$x_s$ and goal~$x_g$ states
and that
...
To understand the differences recall that optimal paths for MTDI follow a ``bang-bang`` controller. Namely, we first apply maximal (or minimal) acceleration for some duration and then switch to applying minimal (or maximal, respectively) acceleration.
...
depicts two informed sets defined using the same cost~$\hat{c}$ using different start and goal states. Notice the difference in structure of~\Cinf. 
Fig~\ref{fig:discont} depicts a simple example where the cost map may is discontinuous.

To summarize, the structure of \Cinf can change given different start and goal states.
Furthermore,  its boundary may not even be smooth or differentiable due to the aforementioned discontinuous. 
...
\begin{figure}[tb]
  ...
  \caption{
    \captionstyle
  	Visualization of the discontinuity in the cost function of MTDI (right) as a function of the controls applied (left). 
  	...
  	However, for states such as $\dot{q}_2$, where $\dot{q}_2 > \dot{q}_{\min}$, we need to apply minimal acceleration  followed by maximal acceleration (curves~$(iv), (v)$), which result in the cost discontinuity.
  	}
   	\label{fig:discont}
\end{figure}
