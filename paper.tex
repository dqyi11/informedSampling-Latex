...
We start by considering a simple yet important motion-planning problem 

The double integrator is a simple yet im-
portant example that nicely illustrates the phase space. Suppose that a second-
order differential equation is given as q  ̈ = u, in which q and u are chosen from R

\section{Problem definition}
\label{sec:pdef}

\subsection{Minimal Time Double Integrator (MTDI)}
In this paper we demonstrate our approach for the specific case of a double integrator system when the objective function is minimizing the trajectory execution time.
