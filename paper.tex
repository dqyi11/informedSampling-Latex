...
%Compared with rejection samplers, a MCMC-based informed sampler takes the advantage of a previous sample in the informed set $ \Cinf $ in finding a new sample, while reserves the randomness in problem space search, which builds an ``exploitation and exploration" patter in informed sampling.
%
...
Hit-and-Run~\cite{S84} MCMC sampler is known to efficiently generate uniform samples. 
Specifically, we use the Accelerated Hit-and-Run variant~\cite{KSZ11} of the algorithm i.e. described in Alg.~\ref{alg:hit_and_run_mcmc}.
...
%\dy{TODO: change MCMC to MHS (Metropolis Hasting Sampler) in the plots}
...
\begin{figure}[t!]
	\includegraphics[width=.46\textwidth]{fig/sampling_efficiency/levelset}
...
\begin{figure}[t!]
	...
	%	\caption{6DoF - 12D. TODO:Time per sample vs informed set}
	%	\label{fig:sampling_efficiency:6d}
	%\end{subfigure}
	 
	\begin{subfigure}[b]{\linewidth}
		\centering
		\includegraphics[width=.85\linewidth]{fig/sampling_efficiency/sample_efficiency_2d}
...
% 	\end{subfigure}	
	\begin{subfigure}[b]{\linewidth}
		\centering
		\includegraphics[width=.85\linewidth]{fig/sampling_efficiency/sample_efficiency_6d}
...
\dy{TODO: Make table for limits and add units}
