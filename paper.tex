...
We conclude our literature review in Sec.~\ref{subsec:mcmc0} with a brief overview of Markov Chain Monte Carlo methods.
...
Here, given the current point $x_i$ the next point~$x_{i+1}$ in the Markov chain is produced by sampling a random direction~$\theta$ on the surface of the unit sphere centered at $x_{i+1}$. This defines a line $\ell_i$ rooted at $x_i$ and passing through~$\theta$. The point $x_{i+1}$ is chosen by randomly sampling a point on $\ell_i$ (see also Sec.~\ref{sec:algorithm}).
