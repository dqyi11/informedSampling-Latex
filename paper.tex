...
%use~$x_\text{boundary}$ 
is used (lines~3-5)
to initialize an MCMC algorithm which produces a series of samples within \Cinf (Sec.~\ref{subsec:mcmc}). 
...
\dy{Basically we follows a MCMC framework for informed sampling, which takes the advantage of a balanced exploration and exploitation.
Under the MCMC framework, we use both Metropolis-Hastings and Hit-And-Run to generate samples.
}
...
(a) we use Metropolis-Hasting 

\begin{algorithm}[t]
	\begin{algorithmic}[1]
		\STATE $ x'_{i+1} \leftarrow g( x'_{i+1} \mid x_{i} ) $
		\STATE $ \alpha = \frac{ P( x'_{i} ) }{ P( x_{i} ) } $ 
		\IF{ $ random() \leq \alpha $ \AND \textsc{Cost}($ x'_{i+1} $) $ < c_{\text{best}} $  }
...
\subsection{Sampling using Hit-And-Run Sampler}

\begin{algorithm}[t]
	\begin{algorithmic}[1]
		\STATE Sample random direction in $ d_i $
		\STATE Find the line set $ L = \{  x \mid x = x_i + \lambda d_i \} $
		\STATE $ \lambda^{+} \leftarrow \sup L $, $ \lambda^{-} \leftarrow \inf L $
		\STATE $ \lambda'_{i} \leftarrow ( \lambda^{-} , \lambda^{+} ) $
		\STATE $ x'_{i+1} = x_{i} + \lambda'_{i} d_i $
		\IF{ \textsc{Cost}($ x'_{i+1} $) $ < c_{\text{best}} $ }
		  \STATE $ x_{i+1} = x'_{i+1} $
 		\ELSE
		  \IF{ $ \lambda'_{i} > 0 $ }
			  \STATE $ \lambda^{+} \leftarrow \lambda'_{i} $
		  \ELSE
			  \STATE $ \lambda^{-} \leftarrow \lambda'_{i} $
		  \ENDIF 
		  \STATE go back to line 4
		\ENDIF 	
	\end{algorithmic}
...
\begin{figure}[t!]
	...
	\end{subfigure}
	\begin{subfigure}[b]{0.5\textwidth}
		\includegraphics[width=\linewidth]{fig/sampling_efficiency/sample_efficiency_4d}
		\caption{4D}
