...
\end{equation}
Consider the obstacles in a workspace, we have $ \Cfree \subseteq \calX $.
Consider the bounds of the control input, we have $ \Ufree \subseteq \calU $.
The dynamics of the robot in Equation~\eqref{eq:kinematics}, free state space $  \Cfree $ and free control input space $ \Ufree $ determines a free trajectory space $ \Pi_{free} $, which contains all feasible trajectories $ \gamma \in \Gamma_{\rm free} $.
An optimal kinodynamic path-planning problem that finds a kinodynamic path minimizing the cost $ \pi^* = \min_{ \pi \in \Gamma_{free} } c( \gamma ) $ subject to constraints $ x(t) \in \Cfree  $ and $ u(t) \in \Ufree $.
...
\os{
	...
	Given some energy function,~$E$, the canonical distribution over states has probability density function $P = \frac{1}{Z}\exp (-E/T)$, where $Z$ is a normalizing constant and $T$ is the temperature of the system.
	...
	Simulating Hamiltonian dynamics requires that the energy function defined is differentiable.
}
